...
subnets=$(aws ec2 describe-subnets --filters "Name=vpc-id,Values=$vpc_id"  --filters="Name=tag:provisioning,Values=manual" --query 'Subnets[*].{SubnetId: SubnetId}' --output text)
for s in ${subnets[@]}; do
